...
const Tree = {
  ...
  init: function () {
    ...
    Tree.width_input.onkeyup = renderOnEnter;
    // @ts-expect-error ts-migrate(2531) FIXME: Object is possibly 'null'.
    Tree.height_input.onkeyup = renderOnEnter;
  },
  ...
  _render: function () {
    ...
    const gdb_root_var_to_update = Tree.gdb_var_being_updated
      ...
      : gdbvar;
    const gdb_var_obj = ExpressionClass.getObjectFromGdbVarName(
      expressions,
  ...
  _get_node_label: function (node: any) {
    ...
    if (node.show_children_in_ui === false && hidden_children > 0) {
      ...
      const child_text = hidden_children === 1 ? "child" : "children";
      ...
      const child_text = node.numchild === 1 ? "child" : "children";
  ...
  _add_nodes_and_edges: function (node: any, parent: any) {
    ...
    const node_label = Tree._get_node_label(node);
    ...
    if (node.name in Tree.nodes._data) {
      ...
      // @ts-expect-error ts-migrate(2339) FIXME: Property 'nodes' does not exist on type '{ el: nul... Remove this comment to see the full error message
      const old_label = Tree.nodes._data[node.name].label;
      const bgcolor = node_label === old_label ? "white" : "yellow";
      // @ts-expect-error ts-migrate(2339) FIXME: Property 'nodes' does not exist on type '{ el: nul... Remove this comment to see the full error message
      Tree.nodes.update({
        id: node.name,
  ...
  render_new_network: function (root_gdb_var_obj: any) {
    ...
    Tree.network.on("click", function (params: any) {
      ...
      const gdb_var_name = this.getNodeAt(params.pointer.DOM);
