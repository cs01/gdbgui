...
export class GdbWebsocket {
  ...
  public runGdbCommand(cmd: Array<string>) {
    ...
    if (debug && commandArray.some((command) => command === undefined)) {
      throw new Error(
        'Developer error: Attempting to send invalid, empty command'
      )
    }
    if (this.socket.connected) {
      ...
      const queuedGdbCommands =
        store.data.queuedGdbCommands.concat(commandArray)
  ...
  public addWebsocketEventHandler(
    eventName: string,
  ...
  private responseTimedOut() {
    ...
    store.set<typeof store.data.waiting_for_response>(
  ...
  constructor(gdbCommand: string, gdbPid: Nullable<number>) {
    ...
    this.socket.on('disconnect', function () {
      ...
      showModal(
        ...
        <>
          <p>
            The connection to the gdb session has been closed. This tab will no
