 ...
 *
 * store is managed in a single location (store._state), and each time the store
 * changes, an event is emitted, which Components listen for. Each Component then re-renders itself
 * as necessary.
 *
 * The store can be changed via store.set() and retrieved via store.get(). store._state should not
 * be accessed directly. store.get() does not return references to objects, it returns new objects.
 * When used this way, store._state is only mutable with calls to store.set().
 *
 ...
 * Debounce functions are used to mitigate inefficiencies of rapid store changes (see _.debounce()).
 *
 */
...
window.store = (function ($, _, Reactor, Awesomplete, Split, io, moment, vis, debug, initial_data) {
"use strict";
...
store.initialize(initial_store)
        ...
        if(obj && store.get('inferior_binary_path')){
            if((obj.last_modified_unix_sec > store.get('inferior_binary_path_last_modified_unix_sec'))
                ...
                Modal.render('Warning', `A source file was modified <bold>after</bold> the binary was compiled. Recompile the binary, then try again. Otherwise the source code may not
                    ...
                    <p>Binary: ${store.get('inferior_binary_path')}, modified ${moment(store.get('inferior_binary_path_last_modified_unix_sec') * 1000).format(DATE_FORMAT)}`)
...
const SourceFileAutocomplete = {
    ...
    fetch_source_files: function(){
        store.set('source_file_paths', [`${ANIMATED_REFRESH_ICON} fetching source files for inferior program`])
    ...
    init: function(){
        ...
        if(!store.get('debug')){
            ...
            $.ajax({
                ...
                error: (data) => {store.set('latest_gdbgui_version', '(could not contact server)')},
            })
        }
    },
...
return store
})(jQuery, _, Reactor, Awesomplete, Split, io, moment, vis, debug, initial_data)
