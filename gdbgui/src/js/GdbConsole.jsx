    ...
    return console_entries.map((entry, index) => {
      switch (entry.type) {
        ...
        case constants.console_entry_type.SENT_COMMAND: {
          let escaped_value = pre_escape(entry.value)
          return (
            <p key={index} className="otpt sent_command pointer" onClick={() => this.props.on_sent_command_clicked(entry.value)}>
              {escaped_value}
            </p>
          )
        }
        case constants.console_entry_type.AUTOCOMPLETE_OPTION: {
          let escaped_value = pre_escape(entry.value)
          return (
            <p key={index} className="otpt autocmplt pointer" onClick={() => this.props.on_autocomplete_text_clicked(entry.value)}>
        ...
        case constants.console_entry_type.UPGRADE_GDBGUI: {
          return (
            <div
              ...
              <span> or </span>
              <a className="btn btn-default btn-xs" style={{color: 'black'}} href={constants.gdbgui_donate_url}>
                {' '}
                donate now.
              </a>
