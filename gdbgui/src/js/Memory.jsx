...
class Memory extends React.Component {
  ...
  constructor() {
    super()
    store.connectComponentState(this, ['memory_cache', 'start_addr', 'end_addr', 'bytes_per_line'])
  }
  get_memory_component_jsx_content() {
    ...
    data.push([
      <span key="moretop" className="pointer" style={{fontStyle: 'italic', fontSize: '0.8em'}} onClick={Memory.click_read_preceding_memory}>
        more
      </span>,
      '',
      '',
    ...
    for (let hex_addr in store.get('memory_cache')) {
      ...
      if (i % bytes_per_line === 0 && hex_vals_for_this_addr.length > 0) {
        // begin new row
        data.push([Memory.make_addrs_into_links_react(hex_addr_to_display), hex_vals_for_this_addr.join(' '), char_vals_for_this_addr])
    ...
    if (Object.keys(store.get('memory_cache')).length > 0) {
      data.push([
        <span key="morebottom" className="pointer" style={{fontStyle: 'italic', fontSize: '0.8em'}} onClick={Memory.click_read_more_memory}>
          more
        </span>,
        '',
        '',
    ...
    if (_.isInteger(start_addr) && end_addr) {
      if (start_addr > end_addr) {
        ...
        Actions.add_console_entries(
          `Cannot fetch ${orig_end_addr - start_addr} bytes. Changed end address to ${store.get('end_addr')} since maximum bytes gdbgui allows is ${
            Memory.MAX_ADDRESS_DELTA_BYTES
          }.`,
