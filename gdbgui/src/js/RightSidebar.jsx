...
class Collapser extends React.Component {
  ...
  render() {
    ...
    if (!this.state.autosize) {
      reset_size_button = (
        <span
          ...
          title={'Height frozen at ' + this.state.height_px + 'px. Click to restore autosize.'}
          style={{
            align: 'right',
            position: 'relative',
            top: '-10px',
    ...
    if (!this.state.collapsed) {
      resizer = (
        <React.Fragment>
          <div className="rowresizer" onMouseDown={this.onmousedown_resizer} style={{textAlign: 'right'}} title="Click and drag to resize height">
            {' '}
            {reset_size_button}
          </div>
        </React.Fragment>
    ...
    return (
      <div className="collapser">
        <div className="pointer titlebar" onClick={this.toggle_visibility.bind(this)}>
          <span className={`glyphicon glyphicon-chevron-${this.state.collapsed ? 'right' : 'down'}`} style={{marginRight: '6px'}} />
          <span className="lighttext">{this.props.title}</span>
        </div>

        <div className={this.state.collapsed ? 'hidden' : ''} id={this.props.id} style={style} ref={n => (this.collapser_box_node = n)}>
          {this.props.content}
        </div>
