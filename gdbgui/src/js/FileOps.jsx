...
const FileOps = {
  ...
  update_source_code_state(fullname, start_line, require_cached_line_num, end_line, assembly_is_cached, file_is_missing, is_paused, paused_addr) {
    ...
    if (fullname && line_is_cached) {
      ...
      is_paused &&
      paused_addr &&
      store.get('disassembly_for_missing_file').some(obj => parseInt(obj.address, 16) === parseInt(paused_addr, 16))
    ) {
  ...
  show_modal_if_file_modified_after_binary(fullname, src_last_modified_unix_sec) {
    if (store.get('inferior_binary_path')) {
      if (src_last_modified_unix_sec > store.get('inferior_binary_path_last_modified_unix_sec') && FileOps.warning_shown_for_old_binary === false) {
        Actions.show_modal(
          'Warning',
          <div>
            This source file was modified <span className="bold">after</span> the binary was compiled. Recompile the binary, then try again. Otherwise
            the source code may not match the binary.
            <p />
            <p>{`Source file: ${fullname}, modified ${moment(src_last_modified_unix_sec * 1000).format(constants.DATE_FORMAT)}`}</p>
            <p>
  ...
  fetch_more_source_at_beginning() {
    ...
    store.set(
      ...
      Math.max(store.get('source_linenum_to_display_start') - Math.floor(store.get('max_lines_of_code_to_fetch') / 2), 1)
    )
