...
const process_gdb_response = function(response_array) {
  ...
  let is_error = response => {
    ...
    is_autocomplete_response = response_array => {
      ...
      if (
        ...
        return gdb_completion_output.every(c => c.type === 'console' && c.stream === 'stdout' && c.message === null)
      } else {
        return false
      }
    }
  ...
  for (let r of response_array) {
    ...
    if (is_error(r)) {
      ...
      } else if (r.token === constants.INLINE_DISASSEMBLY_INT && r.payload && r.payload.msg.indexOf('Mode argument must be 0, 1, 2, or 3.') !== -1) {
      ...
      } else if (r.payload && r.payload.msg && r.payload.msg.startsWith('Unable to find Mach task port')) {
        Actions.add_gdb_response_to_console(r)
        Actions.add_console_entries(
          <React.Fragment>
            <span>Follow </span>
            <a href="https://github.com/cs01/gdbgui/issues/55#issuecomment-288209648">these instructions</a>
    ...
    if (r.type === 'result' && r.message === 'done' && r.payload) {
      ...
      Actions.add_console_entries(r.payload, r.stream === 'stderr' ? constants.console_entry_type.STD_ERR : constants.console_entry_type.STD_OUT)
    ...
    if (r.message && r.message === 'stopped') {
      if (r.payload && r.payload.reason) {
        if (r.payload.reason.includes('exited')) {
          ...
          Actions.add_console_entries(
            `gdbgui noticed a signal was recieved (${r.payload['signal-meaning']}, ${r.payload['signal-name']}).`,
