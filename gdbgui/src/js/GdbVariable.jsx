...
class GdbVariable extends React.Component {
    ...
    let glyph_style = {fontSize: '0.8em', paddingLeft: '5px'},
      delete_button =
        is_root && expr_type === 'expr' ? (
          <span style={glyph_style} className="glyphicon glyphicon-trash pointer" onClick={() => GdbVariable.delete_gdb_variable(mi_obj.name)} />
        ) : (
          ''
        ),
  ...
  static _get_full_path(obj) {
    ...
    function update_path(path, obj) {
      ...
      if (potential_addition === 'public' || potential_addition === 'private' || potential_addition === 'protected') {
