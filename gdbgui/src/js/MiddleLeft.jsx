...
class MiddleLeft extends React.Component {
    ...
    render(){
        return <div id='code_container' style={{overflow: 'auto', height: '100%'}} ref={(el)=>this.source_code_container_node = el}>
                    <SourceCode />
                </div>
    }
    componentDidMount(){
    ...
    check_to_autofetch_more_source(){
        ...
        if(SourceCode.view_more_top_node){
            let {is_visible} = SourceCode.is_source_line_visible($(SourceCode.view_more_top_node))
            if(is_visible){
        ...
        if(!fetching_for_top && SourceCode.view_more_bottom_node){
            let {is_visible} = SourceCode.is_source_line_visible($(SourceCode.view_more_bottom_node))
            if(is_visible){
