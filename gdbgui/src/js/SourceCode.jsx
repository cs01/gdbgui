...
class SourceCode extends React.Component {
  ...
  static _get_assm_content(key, assm, paused_addr) {
    let opcodes = assm.opcodes ? <span className="instrContent">{`(${assm.opcodes})`}</span> : '',
      ...
      asterisk = on_current_instruction ? (
        <span className="glyphicon glyphicon-chevron-right" style={{width: '10px', display: 'inline-block'}} />
      ) : (
  ...
  _get_assm_row(key, assm, paused_addr) {
    return (
      <tr key={key} className="srccode">
        <td className="assembly loc">{SourceCode._get_assm_content(key, assm, paused_addr)}</td>
      </tr>
    )
  }
  ...
  is_gdb_paused_on_this_line(line_num_being_rendered, line_gdb_is_paused_on) {
    if (this.state.paused_on_frame) {
      return line_num_being_rendered === line_gdb_is_paused_on && this.state.paused_on_frame.fullname === this.state.fullname_to_render
    } else {
      return false
    }
  }
  ...
  get_end_of_file_tr(linenum) {
    return (
      <tr key={linenum}>
        <td />
        <td style={{fontStyle: 'italic', paddingLeft: '10px', fontSize: '0.8em'}}>(end of file)</td>
      </tr>
    )
  }
