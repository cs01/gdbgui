...
const BreakpointSourceLineCache = {
  ...
  get_line: function(fullname, linenum) {
    if (BreakpointSourceLineCache._cache['fullname'] !== undefined && _.isString(BreakpointSourceLineCache._cache['fullname'][linenum])) {
...
class Breakpoint extends React.Component {
  ...
  render() {
    ...
    if (b.is_parent_breakpoint) {
      function_jsx = <span className="placeholder">{info_glyph} parent breakpoint on inline, template, or ambiguous location</span>
    } else {
      ...
      function_jsx = (
        <div style={{display: 'inline'}}>
          ...
          <span style={{color: '#bbbbbb', fontStyle: 'italic'}}>thread groups: {b['thread-groups']}</span>
        </div>
      )
    }

    return (
      <div className="breakpoint" onClick={() => Actions.view_file(b.fullname_to_display, b.line)}>
...
class Breakpoints extends React.Component {
  ...
  static delete_breakpoint(breakpoint_number) {
    GdbApi.run_gdb_command([GdbApi.get_delete_break_cmd(breakpoint_number), GdbApi.get_break_list_cmd()])
  }
